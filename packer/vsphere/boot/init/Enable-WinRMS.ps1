...
Function Enable-GlobalHttpFirewallAccess
{
    ...
    If (-not $rule) {
        ...
        $rule.Description = "Inbound rule for Windows Remote Management via WS-Management. [TCP 5986]"
        $add_rule = $true
    }
...
}Else{Write-Output "PS Remoting is already enabled."}
...
If ($DisableBasicAuth) 
{
    ...
    }Else{Write-Output "Basic auth is already disabled."}
} 
Else 
{
    ...
    }Else{Write-Output "Basic auth is already enabled."}
}
