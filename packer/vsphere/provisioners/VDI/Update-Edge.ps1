...
if (!(Test-Path $Folder)) {
  Write-Output "Folder not found. Creating $($Folder)"
...
if (Test-Path $Folder) {
  foreach ($artifacts in $selectedObject.Artifacts) {
   ...
   throw "Folder $Folder does not exist";break
 }
Write-Host "-- Script Completed: File Downloaded -- " -ForegroundColor Green

try{
  Write-Output "Installing $($fileName)"
  Start-Process msiexec.exe -ArgumentList "/i `"$Folder\$fileName`" /passive /norestart DONOTCREATEDESKTOPSHORTCUT=true " -PassThru -Wait
  #msiexec /i "$Folder\$fileName" /passive /norestart DONOTCREATEDESKTOPSHORTCUT=true
}
catch {Write-Error "Edge Update Failed"; throw $error[0]}

try{
  Write-Output "Removing Installer"
  Get-Item "$Folder\$fileName" | Remove-Item
}
catch {Write-Error "Removal of Edge Installer Failed"}